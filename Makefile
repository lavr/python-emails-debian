...
deb:
	...
	dch -M -v ${EMAILS_VERSION}-1 --package python-emails
	cp -r debian python-emails-${EMAILS_VERSION}
	cd python-emails-${EMAILS_VERSION} && dpkg-buildpackage -rfakeroot
	#cd python-emails && dpkg-buildpackage -rfakeroot -uc -us
	# now upload python-emails_${EMAILS_VERSION}-1_all.deb to PPA

upload:
	set -e
	dput ppa:lavrme/python-emails-ppa python-emails_${EMAILS_VERSION}-1_amd64.changes 
